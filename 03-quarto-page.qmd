...
Today, we will explore the data from the [**Palmerpenguins**](https://allisonhorst.github.io/palmerpenguins "palmerpenguins R package") package to answer the following question:

***Comment est la relation entre la longueur des nageoires et la masse corporelle ? Cette relation varie-t-elle selon les espèces ?***

But first, get a grasp of the basics and create our first Quarto document!
...
1.  Create a new Quarto document (`.qmd`) clicking on *`File > New File > Quarto Document`*
...
3.  Now, open the `penguins-template.qmd` in Rstudio
4.  Once Rstudio up and running, copy and paste `install.packages("palmerpenguins")` into the **Console** and run it once
:::
...
### Cross-referencing with `label`  {#label}
...
And so much more... Go see the full [Quarto documentation](https://quarto.org/docs/guide/).
